...
long pipe_close (struct file *filep) {
    ...
    if(filep->mode == O_READ && filep->pipe->pipe_per_proc[index].canRead == 1){
        filep->pipe->pipe_per_proc[index].canRead = 0;
        filep->pipe->pipe_global.processReadCount--;
    }
    else if(filep->mode == O_WRITE && filep->pipe->pipe_per_proc[index].canWrite == 1){
...
int is_valid_mem_range (unsigned long buff, u32 count, int access_bit) {
    ...
    for(int i = 0; i< MAX_MM_SEGS;i++){
        ...
        if(i == 3 && ret_value < 0){ // change this to 3
            //checking in stack 
    ...
    if(ret_value < 0){
        ...
        while (curr_vm_area != NULL)
        {
            if(buff >= curr_vm_area->vm_start && buff <= curr_vm_area->vm_end){
                if(access_bit == 1){
                    if((access_bit & curr_vm_area->access_flags) == 1){
                        ret_value = 1;
                    }
                } 
                else if(access_bit == 2){
                    if((access_bit & curr_vm_area->access_flags) == 2){
                        ret_value = 1;
                    }
                }
            }
...
int pipe_read (struct file *filep, char *buff, u32 count) {
    ...
    if(readOff < writeOff){
        ...
        if(readOff == writeOff && filep->pipe->pipe_global.dataPresent == 0){
            // printk("Cannot read more, because not0hing left to read");
        }
        else {
